...
// Reference - Dennis, Schnabel, Numerical Methods for Unconstrained
// Optimization and Non-Linear Equations

double
cholmod(double* A, int N, double* L, double eps, double maxinp)
{
    /*
   * Algorithm 5.5.2 Dennis, Schnabel, Numerical Methods for Unconstrained
   * Optimization and Non-Linear Equations
   */
...
double
modelhess(double* A, int N, double* dx, double eps, double* L)
{
    /*
   * Algorithm 5.5.1 Dennis, Schnabel, Numerical Methods for Unconstrained
   * Optimization and Non-Linear Equations
   */
    double* U22;
...
int hessian_fd(custom_function* funcpt,
    double* x,
    int N,
    double* dx,
    double eps,
    ...
    for (i = 0; i < N; ++i) {
        ...
        for (j = 0; j < N; ++j) {
            ...
            if (f[i * N + j] >= DBL_MAX || f[i * N + j] <= -DBL_MAX) {
                printf(
                    "Program Exiting as the function value exceeds the maximum double "
                    "value");
                free(xi);
                free(xj);
                free(xij);
...
int hessian_fd2(custom_function* funcpt,
    double* x,
    int N,
    double* dx,
    double eps,
    double* f)
{
    ...
    double *stepsize, *f2;
    /*
      Returns Upper Triangular matrix only. If you want the full matrix then
     either use hessian_fd or copy the upper triangular value to lower triangle.
      */
    fd = pow((double)eps, 1.0 / 3.0);
    ...
    if (fc >= DBL_MAX || fc <= -DBL_MAX) {
        printf("Program Exiting as the function value exceeds the maximum double "
               "value");
        free(stepsize);
        free(f2);
        return 15;
...
void fdjac(custom_gradient* funcgrad,
    double* x,
    int N,
    double* jac,
    double* dx,
...
void hessian_fdg(custom_gradient* funcgrad,
    double* x,
    int N,
    double* jac,
    double* dx,
...
int hessian_opt(custom_function* funcpt,
    custom_gradient* funcgrad,
    double* x,
    int N,
    double* jac,
    double* dx,
...
int lnsrch(custom_function* funcpt,
    double* xi,
    double* jac,
    double* p,
    int N,
    ...
    double* x)
{
    ...
    double alpha, lambda, lambdamin, funcf, funci, lambdaprev, lambdatemp,
        funcprev;
    ...
    while (retval > 1 && iter < MAXITER) {
        ...
        if (funcf <= funci + alpha * lambda * slopei[0]) {
            ...
            if (lambda == 1.0) {
                ...
                temp1[0] = 1.0 / lambda2;
                temp1[1] = -1.0 / lambdaprev2;
                temp1[2] = -lambdaprev / lambda2;
                temp1[3] = lambda / lambdaprev2;
...
int lnsrchmod(custom_function* funcpt,
    custom_gradient* funcgrad,
    double* xi,
    double* jac,
    double* p,
            ...
            if (slopen[0] < beta * slopei[0]) {
                if (lambda == 1.0 && nlen < maxstep) {
                    ...
                    while (funcf <= funci + alpha * lambda * slopei[0] && slopen[0] < beta * slopei[0] && lambda < lambdamax) {
                        ...
                        if (funcf >= DBL_MAX || funcf <= -DBL_MAX) {
                            printf(
                                "Program Exiting as the function value exceeds the maximum "
                                "double value");
                            free(slopei);
                            free(temp1);
                            free(temp2);
...
int lnsrchcg(custom_function* funcpt,
    custom_gradient* funcgrad,
    double* xi,
    double* jac,
    double* p,
...
int newton_min_func(custom_function* funcpt,
    custom_gradient* funcgrad,
    double* xi,
    int N,
    double* dx,
    ...
    double* xf)
{
   ...
   * 2 - Distance between the last two steps is less than stol or |xf - xi| <=
   * stol so xf may be the local minima. 3 - Global Step failed to locate a
   * lower point than xi so xi may be the local minima. 4 - Iteration Limit
   * exceeded. Convergence not achieved.
   *
   */
    ...
    *niter = 0;
    ...
    // maxstep = 1000.0; // Needs to be set at a much higher value proportional to
    // l2 norm of dx
...
int trsrch(custom_function* funcpt,
    double* xi,
    double* jac,
    double* sN,
    int N,
...
int trupdate(custom_function* funcpt,
    double* xi,
    double* jac,
    double* step,
    int N,
...
int newton_min_trust(custom_function* funcpt,
    custom_gradient* funcgrad,
    double* xi,
    int N,
    double* dx,
    ...
    while (rcode == 0 && *niter < siter) {
        *niter = *niter + 1;
        ...
        if (method == 0) {
            ...
            retval = trsrch_ddl(
                funcpt, xc, jac, step, N, dx, maxstep, iter, L, hess, stol, ioval, xf);
        } else {
...
int trsrch_ddl(custom_function* funcpt,
    double* xi,
    double* jac,
    double* sN,
    int N,

...
static void
jrotate(double* A, int N, double a, double b, int i)
{
    int j, r, r1;
    double c, s, den, y, w;
    r = i * N;
...
static void
qrupdate(double* R, int N, double* u, double* v)
{
    int i, j, k, l;
...
static void
inithess_naive(double* H, int N, double fi, double fsval, double* dx)
{
    int i, j, ct;
    double temp;
...
int bfgs_min_naive(custom_function* funcpt,
...
static void
inithess_lower(double* L, int N, double fi, double fsval, double* dx)
{
    int i, j, ct;
    double temp;
...
int bfgs_min(custom_function* funcpt,
    custom_gradient* funcgrad,
    double* xi,
    int N,
    double* dx,
    ...
    double* xf)
{
    ...
    /*
       ...
       * 1 - df(x)/dx <= gtol achieved so xf may be the local minima.
       * 2 - Distance between the last two steps is less than stol or |xf - xi|
     <= stol so xf may be the local minima.
       * 3 - Global Step failed to locate a lower point than xi so xi may be the
     local minima.
...
int bfgs_min2(custom_function* funcpt,
    custom_gradient* funcgrad,
    double* xi,
    int N,
    int m,
    ...
    double* xf)
{
   ...
   * stol so xf may be the local minima. 3 - Global Step failed to locate a
   * lower point than xi so xi may be the local minima. 4 - Iteration Limit
   * exceeded. Convergence not achieved. 15 -Failure as Inf/Nan Values
   * encountered
   *
   */
...
int bfgs_l_min(custom_function* funcpt,
    custom_gradient* funcgrad,
    double* xi,
    int N,
    int m,

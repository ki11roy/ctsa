 ...
 *Function Ratios, ACM Transactions on Mathematical Software, Volume 18, Number
 *3, pages 360-373, 1992.
 */
...
erf__(double x)
{
   ...
   * ----------------------------------------------------------------------- */
    ...
    double a[5] = { 7.7105849500132e-5,
        -.00133733772997339,
        .0323076579225834,
        .0479137145607681,
        .128379167095513 };
    double b[3] = { .00301048631703895, .0538971687740286, .375795757275549 };
    double p[8] = { -1.36864857382717e-7, .564195517478974, 7.21175825088309,
...
erfinv(double x)
{
    ...
    if (a <= 0.75) {
        ...
        double p[5] = { 4.62680202125696,
            -16.6805947126248,
            17.6230176190819,
            -5.4309342907266,
            0.236997019142 };
        double q[5] = { 4.26606447606664,
            -17.5930726990431,
            22.7331464544494,
            -9.9016863476727,
            1.000000000000 };
        ...
        /*double p[7] =
       {0.00479447793346489,0.17121766901388401,1.26186520719596240,\
                     2.1339060556433715,0.2838737490019514,0.2353035904898929,\
                     -0.0400763898644416};
             double q[6] =
       {0.00479437046609729,0.17124542554915192,1.27422170654959402,\
                     2.4161883799488509,1.4957337483050566,1.0000000000000000};

             t=1.0/sqrt(-log(1.0-a));
             num = p[0] + t * ( p[1] + t * ( p[2] + t * ( p[3] + t * ( p[4] + t
       * ( p[5] + t * ( p[6] + t * p[7])))))); den = q[0] + t * ( q[1] + t * (
       q[2] + t * ( q[3] + t * (q[4] + t * ( q[5] + t * q[6]))))); oup = sign *
       num / (den * t);
             * */
        ...
        t = 1.0 / sqrt(-log(1.0 - a));
        oup = sign * (p[0] / t + p[1] + t * (p[2] + t * (p[3] + t * (p[4] + t * p[5])))) / (q[0] + t * (q[1] + t * (q[2])));
    } else {
        oup = sign * xinf;
    }
...
erfcinv(double x)
{
    ...
    if (x >= 0.0625 && x < 2.0) {
        ...
        t = 1.0 / sqrt(-log(x));
        oup = (p[0] / t + p[1] + t * (p[2] + t * (p[3] + t * (p[4] + t * p[5])))) / (q[0] + t * (q[1] + t * (q[2])));

    } else if (x < 1.0e-100 && x > 1.0e-1000) {
        // Coefficients from table 70
        double p[4] = {
            0.00980456202915, 0.363667889171, 0.97302949837, -0.5374947401
        };

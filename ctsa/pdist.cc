...
double
normalpdf(double x, double mu, double sigma)
{
    double oup, t;
...
double
normalinv(double p, double mu, double sigma)
{
    double oup, t;
    if (sigma < 0.) {
        printf("Standard Deviation (Sigma) must be non-negative");
...
tinv_appx(double p, int df)
{
   ...
   */
    ...
    if (p > 0. && p < 1.0) {
        ...
        g3 = (3. * pow(x, 7.0) + 19. * pow(x, 5.0) + 17. * pow(x, 3.0) - 15. * x) / (384. * pow(d, 3.0));
        g4 = (79. * pow(x, 9.0) + 776. * pow(x, 7.0) + 1482. * pow(x, 5.0) - 1920. * pow(x, 3.0) - 945. * x) / (92160. * pow(d, 4.0));
        t = x + g1 + g2 + g3 + g4;
    }
...
double
gammapdf(double x, double k, double th)
{
    double oup, t;
    /*
...
double
gammacdf(double x, double k, double th)
{
    double oup, t;
...
double
gammainv(double p, double k, double th)
{

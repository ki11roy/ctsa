...
arima_object
arima_init(int p, int d, int q, int N)
{
...
sarima_object
sarima_init(int p, int d, int q, int s, int P, int D, int Q, int N)
{
...
void sarima_predict(sarima_object obj,
    double* inp,
    int L,
    double* xpred,
    double* amse)
...
int ar_estimate(double* x, int N, int method)
{
    ...
    for (i = 0; i < ordermax; ++i) {
        if (method == 0) {
            ...
            as154(
                inp, N, 7, i + 1, 0, 0, phi, NULL, &wmean, &var, resid, &loglik, hess);
        } else {
...
void model_estimate(double* x, int N, int d, int pmax, int h)
{
    ...
    /*
      ...
      d - Number of time the series has to be differenced (d = 0 when there are
     no trends) pmax - Maximum AR and MA order pmax = max(p,q) h - Order of AR
     model to be fitted to obtain residuals 0 <= pmax <= h Typically pmax = 3,4
     and h = 8. Increase value of h if you want to fit high order ARMA process
      */
...
// Yule-Walker, Burg and Hannan Rissanen Algorithms for Initial Parameter
// Estimation

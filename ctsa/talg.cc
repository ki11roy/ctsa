...
void arma_autocovar(double* phi,
    double* theta,
    int p,
    int q,
    double var,
...
int twacf(double* P,
    ...
    int MXPQ)
{
    int ifault, i, k, kc, j, jpk, kcp1mj, j1, kp1, kp2mj, miim1p, imj, mikp,
        kp1mj;
...
void arinvtrans(int p, double* oldv, double* newv)
{
    ...
    for (j = 0; j < p; ++j) {
        temp[j] = newv[j] = oldv[j];
    }

    for (j = p - 1; j > 0; --j) {
        a = newv[j];
        for (k = 0; k < j; ++k) {
            temp[k] = (newv[k] + a * newv[j - k - 1]) / (1 - a * a);
        }
        for (k = 0; k < j; ++k) {
            newv[k] = temp[k];
        }
...
void transall(int p, int q, int P, int Q, double* oldv, double* newv)
{
    ...
    if (q != 0) {
        artrans(q, oldv + N, newv + N);
        N = N + q;
    }

    if (P != 0) {
        artrans(P, oldv + N, newv + N);
        N = N + P;
    }
...
void invtransall(int p, int q, int P, int Q, double* oldv, double* newv)
{
    ...
    if (p != 0) {
        arinvtrans(p, oldv, newv);
        N = N + p;
    }

    if (q != 0) {
        arinvtrans(q, oldv + N, newv + N);
        N = N + q;
    }

    if (P != 0) {
        arinvtrans(P, oldv + N, newv + N);
        N = N + P;
    }

...
/*
 * C Routine nel_min is based on O'Neill's FORTRAN implementation of Nelder-Mead
 algorithm
 * Algorithm AS 47: Function Minimization Using a Simplex Procedure
 ...
 Statistics), Vol. 20, No. 3 (1971), pp. 338-345 Published by: Blackwell

        Main Reference

        J. A. Nelder and R. Mead, A simplex method for function minimization.
 Compuer Journal,7,308-313

 */

int nel_min(custom_function* funcpt,
    double* xc,
    int N,
    double* dx,
    double fsval,
    ...
    while (ctl == 0 && *niter < siter) {
        ...
        while (fabs(s) > reqmin && *niter < siter) {
            *niter = *niter + 1;
            ...
            s = sqrt(s); // Remove this statement if you want convergence in fewer
                // iterations
            if (fabs(s) < reqmin) {
                ctl = 1;
            }

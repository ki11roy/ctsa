...
static void
mcon(double* ETA, double* INFINY, double* SMALNO, double* BASE)
{
    *ETA = macheps();
    *INFINY = DBL_MAX;
    *SMALNO = DBL_MIN;
...
static double
CAUCHY(int NN, double* PT, double* Q)
{
    /*
            ...
            if (CMOD(&tp1, &tp2) < 0.5 * CMOD(ZR, ZI)) {
                if (PASD == 1) {
                    ...
                    VRSHFT(NN,
                        ...
                        TI);
                    if (*CONV == 1) {
                        break; // C THE ITERATION FAILED TO CONVERGE. TURN OFF TESTING AND
                            // RESTORE
                    }
                    TEST = 0;
                    for (i = 0; i < N; ++i) {
                    ...
                    *SI = SVSI;
                    POLYEV(NN, SR, SI, PR, PI, QPR, QPI, PVR, PVI);
                    CALCT(
                        &BOOL, NN, SR, SI, HR, HI, QHR, QHI, ARE, INFIN, PVR, PVI, TR, TI);
                } else {
                    PASD = 1;
                }
...
{
    /*
      Input - coeff[0] + coeff[1] * x^1 + coeff[2] * x^2 + ---- +
     coeff[DEGREE+1] * x^DEGREE
...
{
    /*
      Input - rcoeff[0]+i*icoeff[0] + (rcoeff[1] + i*icoeff[1]) * x^1 +
     (rcoeff[2] + i*icoeff[2]) * x^2 + ---- + (rcoeff[DEGREE+1] +
     i*icoeff[DEGREE+1]) * x^DEGREE

      The program fails if there are leading zeros [At least one coefficient of
     the highest degree should be non-zero. Reduce degree by one if both are
     zero]. Check return value.

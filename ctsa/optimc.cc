...
int fminunc(custom_function* funcpt,
    ...
    double* xf)
{
   ...
   * Codes 0, 4 and 15 denote failure. [Code 4 my also occur in cases where the
   * minima is realized but convergence is not achieved due to tolerance values
   * being too small. It is recommended that you use another method if you
   * suspect that such a scenario has occured.]
   *
   * 0 - Input Error
   ...
   * stol so the point may be the local minima. 3 - Global Step failed to locate
   * a lower point than the current point so it may be the local minima. 4 -
   * Iteration Limit exceeded. Convergence probably not achieved. 6 - Function
   * value drops below ftol (relative functional tolerance). 15 - Overflow
   * occurs. Try a different method.
   *
   */
    fsval = 1.0;
    MAXITER = 200 * N;
    ...
    if (method == 0) {
        ...
        retval = newton_min_trust(funcpt,
            funcgrad,
            xi,
            N,
            dx,
        ...
        retval = conjgrad_min_lin(funcpt,
            funcgrad,
            xi,
            N,
            dx,
        ...
        printf(
            "Method Value should be one of 0,1,2,3,4,5,6 or 7. See Documentation. "
            "\n");
        exit(1);
    }
...
double
fminbnd(custom_funcuni* funcuni, double a, double b)
{
    double x, t, eps;
...
int fminnewt(custom_function* funcpt,
    custom_gradient* funcgrad,
    int N,
    double* xi,
    double delta,
...
void optimize(opt_object obj,
    custom_function* funcpt,
    custom_gradient* funcgrad,
    int N,
    double* xi,
    int method)
{
   ...
   * minima is realized but convergence is not achieved due to tolerance values
   * being too small. It is recommended that you use another method if you
   * suspect that such a scenario has occurred.]
   *
   * 0 - Input Error
   * 1 - df(x)/dx <= gtol achieved so current point may be the local minima.
   * 2 - Distance between the last two steps is less than stol or |xf - xi| <=
   ...
   */
    ...
    if (obj->N != N) {
        ...
        printf("Please Reinitialize the object for the new size %d using opt_init "
               "command \n",
            N);
        exit(1);
    }
    ...
    if (method == 0) {
        ...
        obj->retval = newton_min_func(funcpt,
            funcgrad,
            xi,
            obj->N,
            dx,
        ...
        obj->retval = newton_min_trust(funcpt,
            funcgrad,
            xi,
            obj->N,
            dx,
        ...
        obj->retval = conjgrad_min_lin(funcpt,
            funcgrad,
            xi,
            obj->N,
            dx,
...
int levmar(custom_funcmult* funcmult,
    custom_jacobian* jacobian,
    double* xi,
    int M,
    int N,
    double* xf)
...
void nls(nls_object obj,
    custom_funcmult* funcmult,
    custom_jacobian* jacobian,
    double* xi)
{
...
void nlssummary(nls_object obj)
{
    ...
    if (obj->retval == 1) {
        printf(
            "Termination Message : both actual and predicted relative reductions "
            "in the sum of squares are at most ftol. \n");
    }
    if (obj->retval == 2) {
        printf(
            "Termination Message : relative error between two consecutive iterates "
            "is at most xtol. \n");
    }
    if (obj->retval == 3) {
        printf(
            "Termination Message : conditions for info = 1 and info = 2 both hold. "
            "\n");
    }
    if (obj->retval == 4) {

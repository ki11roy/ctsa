...
reg_object
reg_init(int N, int p)
{
    reg_object obj = NULL;
    int i;
    /*
   * reg_init initializes the regression object.
   * N - number of observation samples. All variables (indepnedent and
   * dependent) are of identical length N.
   *
    ...
    double* ci_upper)
{
   ...
   */
    ...
    *sigma2 /= (double)df;
    ...
    R2[1] = 1.0 - (1.0 - R2[0]) * ((double)N - 1.0) / ((double)df);
    ...
    double alpha)
{
   ...
   *  x - Vector containing (p - 1 ) independent regression variables of length
   * N each y - Vector containing dependent regression variable of length N res
   * - Residual vector of length N varcovar - Variance-Covariance Matrix of p
   * regression parameters. Dimension p X p alpha is used to determine
   * confidence interval limits for a given 100*(1-alpha) % confidence interval
   * eg., alpha = 0.05 if you want to determine 95% confidence interval values
   */
    p = obj->p;
    obj->alpha = alpha;
    anv2 = (double*)malloc(sizeof(double) * 7);
    b2 = (double*)malloc(sizeof(double) * p);
...
void regress_poly(reg_object obj,
    double* x,
    double* y,
    double* res,
    double* varcovar,
...
void summary(reg_object obj)
{
    ...
    printf(
        "R-Squared = %lf , Adjusted R-Squared = %lf \n", obj->R2[0], obj->R2[1]);
}
...
void anova(reg_object obj)
{
    ...
    printf("%-25s%-20d%-20lf%-20lf \n",
        ...
        obj->df_ESS,
        obj->ESS,
        obj->ESS / ((double)obj->df_ESS));
    printf("%-25s%-20d%-20lf%-20lf \n",
        "Due to Residual",
        obj->df_RSS,
        obj->RSS,
        obj->RSS / ((double)obj->df_RSS));
    printf("%-25s%-20d%-20lf \n", "Total", obj->df_RSS + obj->df_ESS, obj->TSS);
    printf("\n F Statistics = %g \n", obj->FStat);
    printf("\n P Value (F)  = %g \n", obj->PVal);
}
...
void confint(reg_object obj)
{
    ...
    printf("\n");
    printf(
        "%-10lf%% Confidence Interval For Parameters And Residual Variance : \n",
        (1.0 - obj->alpha) * 100);
    printf("%-25s%-20s%-20s%-20s \n",
        "Parameters",
        "Value",
...
double
fitted(reg_object obj, double* inp, double* varcovar, double* var)
{
    int i, p;
    double oup;
    double *y, *x0, *b, *temp;
    /*
   * The function fitted fits input values one vector at a time and outputs one
   * output value
   *
   * inp - ia a  vector of length p-1 values corresponding to each of the p-1
   * dependent variables.
   * The function returns output value corresponding to the input inp vector of
   * length (p-1).
   *
   * varcovar - Is the Aurovariance-Covariance matrix that is obtained from the
   * regression function var - is a vector of length 2 where var[0] - Variance
   * of Mean Prediction var[1] - Variance Of Individual Prediction
   *
   */
    p = obj->p;
...
void linreg_multi2(int p, double *xi,double *y, int N, double* b) {
        ...
        double LWKOPT;
         ...
         //infoi =
LAPACKE_dgetri_work(LAPACK_ROW_MAJOR,p,xxt,lda,ipiv,&LWKOPT,MONE);
         ...
         //infoi =
LAPACKE_dgetri_work(LAPACK_ROW_MAJOR,p,xxt,lda,ipiv,WORK,LWORK);

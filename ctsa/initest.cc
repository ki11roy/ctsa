...
void innalg(double* vec, int l, int N, double* v, double* theta)
{
    ...
    // theta is a (N+1)X(N+1) matrix so choose the value of N accordingly. The
    // recommended value is N = 17 which will make theta a 18*18 double vector so
...
void burgalg(double* x, int N, int p, double* phi, double* var)
{
    ...
    /*
      This code is the C translation of Cedrick Collomb's C++ Implemnetation of
     Burg's Algorithm with some minor modifications A tutorial on linear
     prediction and the Burg method. http://www.emptyloop.com/technotes/
      */
...
void pacf_mle(double* vec, int N, double* par, int M)
{
    ...
    for (i = 0; i < M; ++i) {
        ...
        ret = nlalsm(
            vec, N, p, 0, 0, phi, theta, mean, &thetac, &var, eps, varcovar, res);
        // ret = css(vec, N, 6, p, 0, 0, phi, theta, &thetac, &var, res, &loglik,
        // varcovar);
        par[i] = phi[i];
        free(phi);
        free(varcovar);
